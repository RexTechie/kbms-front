    ...
    <nav class="site-navbar" :class="'site-navbar--' + navbarLayoutType">
      <div class="site-navbar__header">
        <h1 class="site-navbar__brand">
          <a class="site-navbar__brand-lg">企业知识库系统</a>
          <a class="site-navbar__brand-mini">KBMS</a>
        </h1>
      </div>
          ...
          <el-menu-item class="site-navbar__avatar" index="3">
            <el-dropdown>
              <span class="el-dropdown-link">
                <img :src="`${baseUrl}/static/img/avatar/avatar.png`"  alt=""/>
                {{ name }}
              </span>
              <template #dropdown>
                <el-dropdown-menu>
...
export default {
  ...
  methods: {
    logout: function () {
      let that = this;
      that.$http("/logout", "GET", null, true, function (resp) {
  ...
  mounted: function () {
    let that = this;
    //加载用户数据
    that.$http("/user/current", "GET", null, true, function (resp) {
      if(resp.code === 200) {
        let json = resp.data;
        that.name = json.nickName;
      }else{
